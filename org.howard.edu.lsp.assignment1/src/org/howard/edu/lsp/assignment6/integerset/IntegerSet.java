...
public class IntegerSet {
	...
	public void union(IntegerSet intSetb) {
		...
		for (int x = 0;x < intSetb.length(); x++)
			new_map.put(intSetb.get(x), true);
				...
				for (int x = 0;x < set.size(); x++)
			new_map.put(set.get(x), true);
		set.clear();
		set.addAll(new_map.keySet());
	...
	public void intersect(IntegerSet intSetb) {
		...
		for (int x = 0; x < intSetb.length(); x++)
			map.put(intSetb.get(x), false);
		
		for (int x = 0; x <set.size(); x++) {
			if (map.containsKey(set.get(x)))
				map.put(set.get(x), true);
			else
				map.put(set.get(x), false);
		}
		set.clear();
